...
The `.harness/ledger.jsonl` file logs every operation including:
...
Rules in `.harness/rules/*.md` may contain:
- Security policies
- API endpoints
...
The `harness intercept` hook (planned for v0.2) will run on every Claude Code prompt. Ensure:
- Harness binary is from trusted source
- `.claude/settings.json` hook command is not modified maliciously
- File permissions on `.harness/` prevent unauthorized modification

### Policy Files

`.harness/policy.yaml` controls model tier selection. A malicious policy could:
...
`harness uninstall` warns before deleting ledger data. This prevents:
...
1. **Keep harness updated** to latest version
2. **Review `.harness/` contents** before committing
...
None currently. Check [GitHub Issues](https://github.com/YOUR_USERNAME/harness/issues) for reported vulnerabilities.
...
For general issues, use [GitHub Issues](https://github.com/YOUR_USERNAME/harness/issues).
